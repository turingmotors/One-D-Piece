...
import torch
from evaluator import ImageClassificationQualityEvaluator
...
def process_buffer(buffer, device, evaluators):
    original_labels = torch.cat([x[0] for x in buffer], dim=0).to(device)
    original_images = torch.cat([x[1] for x in buffer], dim=0).permute(0, 3, 1, 2).to(torch.float) / 255.0
    eval_images = torch.cat([x[2] for x in buffer], dim=0).permute(0, 3, 1, 2).to(torch.float) / 255.0

    for evaluator in evaluators:
        evaluator.update(original_labels, eval_images)
...
def eval_reconstruction(
    ...
    for label, original_image, eval_image in tqdm(zip(labels, original_images, target_images)):
        ...
        buffer.append((label, original_images, eval_images))
...
def main(args):
    ...
    eval_scores = eval_reconstruction(
        ...
        device="cuda",
        evaluators=[
            ImageClassificationQualityEvaluator(
                device="cuda",
            ),
        ]
    )
