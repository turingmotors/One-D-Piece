...
import torch.utils.data.utils as utils
...
class EvalDataset(ImageFolder):
    ...
    def __getitem__(self, item):
        ...
        predict = np.array(Image.open(os.path.join(self.pred_lst[item])), dtype=np.uint16)
...
class EvalDatasetSingleDir(Dataset):
    def __init__(self, pred_root, label_root, name_list, match=None):
    ...
    def __getitem__(self, item):
        ...
        predict = np.array(Image.open(self.pred_lst[item]), dtype=np.uint16)
        predict = predict[:, :, 1] * 256 + predict[:, :, 0]
